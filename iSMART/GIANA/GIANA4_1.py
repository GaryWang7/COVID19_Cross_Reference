...
## Updated on 2021: (Local user Gary) Remove the resource package since it is only run on linux. Replace it with psutil in the
# future. Renamed the file as GIANA4_1 instead of GIANA4.1 due to importing issues
## Updated on April 2022: increase the -t default to 10 and
# cmd: python GIANA4_1.py -f M1_MART1_TSCAN_GIANA.tsv -v -S 3.6
...
def EncodeRepertoire(inputfile, outdir, outfile='', exact=True, ST=3, thr_v=3.7, thr_s=3.5, VDict={}, Vgene=True,
    ...
    if len(outfile) == 0:
        ...
        outfile = outdir + '/' + re.sub('\\.[txcsv]+', '', outfile) + '-' + '-RotationEncodingBL62_'+'S_'+str(thr_s)+'.txt'
    g = open(outfile, 'w')
...
''')
    ...
    parser.add_option("-t", "--threshold", dest="thr", default=10,
