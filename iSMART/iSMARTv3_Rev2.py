...
## Apr 18t, 2022, revised by Gary Wang so that VgeneScore is correctly output to the direction specified by -o option
...
## cmd line: python iSMARTv3_Rev2.py -f tcr_info_iSMART.tsv -t 3 -o./iSMART/
...
def PreCalculateVgeneDist(VgeneFa="Imgt_Human_TRBV.fasta",OutDir = './'):
    ...
    gg=open(OutDir+'VgeneScores.txt','w')
    for kk in VScore:
        vv=VScore[kk]
...
def main():
    ...
    VFa=opt.VFa
    PreCalculateVgeneDist(VFa,OutDir)
    vf=open(OutDir+'VgeneScores.txt')  ## Use tcrDist's Vgene 80-score calculation
    VScore={}
    while 1:
        line=vf.readline()
        if len(line)==0:
